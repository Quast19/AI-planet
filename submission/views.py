...
class CreateSubmission(APIView):
    ...
    def post(self,request):
        ...
        try:
            ...
            elif hackathon_obj.submission_type == "Datatype123":
                submission_obj = Submission.objects.create(hackathon = hackathon_obj, owner = request.user, type = hackathon_obj.submission_type, Datatype123=data['Datatype123'])
                serialized_data = SubmissionSerializer(submission_obj).data
                return Response({'success':True, 'message':'Successfully submitted','submission':serialized_data}, status=status.HTTP_202_ACCEPTED)
